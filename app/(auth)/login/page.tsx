...
function getRedirect(value: string | null) {
  return value === '/mapa' || value?.startsWith('/mapa/') ? value : '/mapa'
}
...
function LoginForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.replace(getRedirect(searchParams.get('redirect')))
      router.refresh()
    } catch (cause) {
...
export default function LoginPage() {
  return (
    <Suspense fallback={<main style={{ minHeight: '100dvh', background: '#20281d' }} />}>
      <LoginForm />
    </Suspense>
  )
}
